...
void list_reverse(struct list_node **head) {
    ...
    while (current != NULL) {
        next = current->next;  // Memorizza il prossimo nodo
        current->next = prev;  // Inverte il puntatore del nodo corrente
        prev = current;        // Sposta prev e current di un nodo in avanti
        current = next;
    }

    *head = prev; // Aggiorna la testa della lista al nuovo primo nodo
}
