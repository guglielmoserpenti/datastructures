...
struct aiv_dict_item {
    const char *key;
    void *value; // Puntatore a qualsiasi tipo di dato
    size_t key_len;
    size_t value_len; // Lunghezza del valore
};
...
void aiv_dict_insert(struct aiv_dict *dict, const char *key, const void *value, size_t value_len) {
    ...
    for (size_t i = 0; i < HASHMAP_SIZE_LIST; i++) {
        if (dict->hashmap[index][i].key_len == 0) {
            dict->hashmap[index][i].key = _strdup(key); // Duplica la chiave
            dict->hashmap[index][i].value = malloc(value_len); // Alloca memoria per il valore
            memcpy(dict->hashmap[index][i].value, value, value_len); // Copia il valore
...
void aiv_dict_remove(struct aiv_dict *dict, const char *key) {
    ...
    for (size_t i = 0; i < HASHMAP_SIZE_LIST; i++) {
        if (dict->hashmap[index][i].key_len == key_len && !memcmp(dict->hashmap[index][i].key, key, key_len)) {
            free((void *)dict->hashmap[index][i].key); // Libera la memoria per la chiave
